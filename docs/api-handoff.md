...
| `addQuestionSet({ name, description, fileName? })` | mutate                            | CPEA "Add new": persists a saved question set. The mock parses no file (placeholder question list, labeled honestly); the real impl parses the CSV/XLSX (one question per row) into `fields`. |
| `getQuestionSets()`                             | read                                 | ERM (v1.5): the selectable question-set configs — `{ id, name, fields: [{ id, label, question, outputType, visible, derivedAcceptable }] }`. `visible` marks monitor-table columns; `derivedAcceptable` is a governance hook (carried, unenforced in the UI this round). |
| `resolvePopulation(criteria)`                   | read                                 | ERM: resolve a population WITHOUT starting a run (the start screen's scope preview). Returns `{ population, documentCount }` where population carries full accounting: `included`, `excluded` (each with a NAMED reason — never silent), `indeterminate` (reported, never defaulted). |
| `startRun({ questionSetId, prompt?, criteria, documents? })` | mutate                  | ERM: start a run; returns `{ runId }` immediately. The run advances server-side (the UI may leave); explicit `documents` union with the population's scope. |
| `getRun(runId)` / `listRuns()`                  | read (poll while running)            | ERM: a run is a RECORD — every state (`queued/running/completed/cancelled/failed`) kept and revisitable, nothing overwrites. Running runs carry `progress` (staged: population resolved → index current → questions with counts → computing). Completed runs carry the full per-answer store: `{ rxm, questionId, value, grade: stated\|derived\|unsupported, conf, inferredFrom?, evidenceRefs, rationale?, limitations? }`. THE GRADING CONTRACT: `stated` only with an entailing quote (evidenceRefs non-empty), `derived` only with `inferredFrom` naming stated inputs, else `unsupported`; retrieval similarity never grades; grades live only on answers (row flag counts are computed client-side). |
| `cancelRun(runId)`                              | mutate                               | ERM: a running run becomes `cancelled` (with `cancelledAt`) and STAYS in Runs as a record. |
