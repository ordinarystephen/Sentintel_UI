...
| ----------------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `me()`                                          | read                                 | Fixed for the session; identity from the firm's credential management system, `name` display-formatted "Last, First". Cached by the UI. `entitlements` lists the applications the user may enter (`crr`, `erm`, `vantage`, `inquiry`) — one entitlement sends `/` straight into that application. **Mock only:** `localStorage['sentinel.mock.user'] = 'u-leadership'` signs in as the Inquiry-only leadership user; the real backend takes identity from the session and has no such key. |
...
| `createReview(input)`                           | mutate                               | v1.6 contract: `{ files, contextText, repositoryDocIds?, config? }` — uploads and repository picks (index-store docIds) mix; a repository-only review (zero uploads) is valid. `config` is the initiation-time WORKPAPER CONFIGURATION (`{ template, sections, customTopics? }` — placeholder vocabulary this round) and rides the review's evidence snapshot (`Review.workpaperConfig`). The former extraction settings (parser/preset/concurrency) were REMOVED entirely — parsing is the platform's job; the http impl must not resurrect them. |
...
| `searchRepository(query, scope?)`               | read                                 | THE shared repository search (re-signed v1.5 as the platform picker contract — ERM's picker and Documents lens today, the future CRR picker's too). `query` matches borrower/counterparty name or RXM — case-insensitive substring, RXM with or without the `RXM-` prefix; empty query returns the scoped list. `scope.rxm` restricts to one borrower; `scope.notInReviewId` additionally drops documents already in that review's evidence set (CRR amend-evidence). One store, many lenses: `repoId` IS the index-store `docId`. Each row: `{ repoId, rxm, counterparty, fileName, docType, uploadedAt, pages, parsed, docId? }`. |
| `listPolicyDocs()`                              | read                                 | Policy search (v1.6): the browsable rows `{ id, title, revisedOn }`, newest revision first. A non-question search term filters these client-side. |
| `askPolicies(question)`                         | read (CAPABILITY PREVIEW)            | Policy search's Ask: returns `{ answer, quote, citation, revisedOn }`. The answering engine is FUTURE WORK — the mock returns the one fixture answer; the http impl should route to the policy-QA service when it exists and keep this exact shape. |
| `getQuestionSets(store)`                        | read                                 | One application's question-set shelf (v1.8 — `store` is `'erm'` for CPEA, `'vantage'` for Vantage). **Stores are per-application: a set saved in one never appears in another.** Each set: `{ id, name, description, fields: [{ id, label, question, outputType, visible, derivedAcceptable }] }`. `visible` marks monitor-table columns; `derivedAcceptable` is a governance hook (carried, unenforced in the UI). |
| `addQuestionSet(store, { name, description, fileName?, questions? })` | mutate        | Saves a set to THAT store only ("Add new", and Vantage's "Save as a question set"). With `questions` (a file the user already reviewed — possibly with some questions removed) those become the set's `fields`, in order; a `questions` list with nothing in it → reject (a set is never invented). Without them the real impl parses the uploaded file (see *The question-file contract* below); the mock parses nothing and stores a placeholder list, labeled honestly. Blank `name` → reject. |
| `parseQuestionFile(file)`                       | read (nothing retained)              | v1.8: reads a question file BACK for review before anything runs — returns `{ fileName, questions: string[] }`. Retains nothing server-side: the client runs the questions once, or saves them via `addQuestionSet`. Wrong type / empty file → reject with a message naming the file. See *The question-file contract*. (`placeholder: true` on the result is the MOCK saying its list is a labeled placeholder — a real parse never sets it.) |
| `searchBorrowers(query)`                        | read                                 | v1.8: the borrower index behind the population zone's borrower scope — `[{ rxm, name, documentCount }]`, where `documentCount` is ALL of that borrower's documents on system. Ratified search keys: name or RXM, case-insensitive substring, RXM with or without the `RXM-` prefix; empty query = the whole index. |
| `resolvePopulation(criteria)`                   | read                                 | Resolve a population WITHOUT starting a run (the start screen's scope preview). Returns `{ population, documentCount }` where population carries full accounting: `included`, `excluded` (each with a NAMED reason — never silent), `indeterminate` (reported, never defaulted). **Borrower scope (v1.8):** `criteria.borrower = { rxm, name }` makes the population exactly that borrower — `included: [borrower]`, nothing excluded or indeterminate, `population.criteria.borrower` set (the accounting names it) — and `documentCount` = all of its documents on system. The four dropdown criteria are carried but not applied while a borrower is set. Unscoped behaviour is unchanged. |
| `startRun(app, { questionSetId?, prompt?, criteria, documents? })` | mutate               | Start a run in ONE application's run store (`app`: `'erm'` = CPEA, `'inquiry'` = Inquiry). Exactly one of `questionSetId` (a set run) or `prompt` (a **prompt-only run: exactly one question**, answers keyed `questionId: 'prompt'`) — neither, or both, rejects. Inquiry has question sets switched off: a `questionSetId` there rejects. Returns `{ runId }` immediately; the run advances server-side (the UI may leave). The population's document scope (a borrower scope: that borrower's documents) unions with explicit `documents`. |
| `getRun(app, runId)` / `listRuns(app)`          | read (poll while running)            | That application's runs only — a runId from another application's store does not resolve. A run is a RECORD — every state (`queued/running/completed/cancelled/failed`) kept and revisitable, nothing overwrites. Running runs carry `progress` (staged: population resolved → index current → questions with counts → computing). Completed runs carry the full per-answer store: `{ rxm, questionId, value, grade: stated\|derived\|unsupported, conf, inferredFrom?, evidenceRefs, rationale?, limitations? }`. THE GRADING CONTRACT: `stated` only with an entailing quote (evidenceRefs non-empty), `derived` only with `inferredFrom` naming stated inputs, else `unsupported`; retrieval similarity never grades; grades live only on answers (row flag counts are computed client-side). A run with exactly one question renders the single-question shape (Borrower · RXM · Flags · Answer); more than one, the portfolio monitor. |
| `cancelRun(app, runId)`                         | mutate                               | A running run becomes `cancelled` (with `cancelledAt`) and STAYS in that application's Runs as a record. |

**ERM export (contract open):** the results screen renders "Export summary (Excel)" and "Export summary (Word)"; neither is wired — the Excel-vs-Word ruling is deliberately open (ledger). When ruled, the export takes a `runId` and renders the summary + monitor table of that frozen run; follow `exportReview`'s blob-download shape. Do not implement one format speculatively.
...
Seam: `askDocuments(questions, documents) → { runId }` (returns immediately; the run advances server-side), `getVantageRun(runId)` (poll while queued/running; carries `progress: { docsRead, answering, statusLine }`), `listVantageRuns()` (newest first — every run kept), `cancelVantageRun(runId)` (kept as `cancelled`).

**A run carries one or many questions (v1.8):** `{ runId, questions: string[], documents: [{ name, kind: pdf|docx|xlsx|csv, meta, sizeBytes? }], state: queued|running|completed|cancelled|failed, sections: [{ question, blocks }], startedAt, cancelledAt? }`. `questions` arrive in the order the user combined them (a typed question first, then a reviewed file's rows; or a saved set's questions). The answer is an **ordered list of sections, one per question, in `questions` order** — each section is `{ question, blocks }`, and **the block contract below applies to every section's `blocks`, unchanged**: same types, same rules, same block-level citations, same unknown-type fallback. `sections` is empty until the run completes. The UI renders one question exactly as v1.7 did (the question as the page heading, its blocks beneath — guarded by a byte-exact regression test); with more than one, each section gets a serif question heading ("Q n of N") above its blocks. A follow-up is a **new single-question run** against the same docset — no context is carried and no API may imply it is.
...
## The question-file contract (v1.8)

Vantage's one-off intake and every shelf's Add new accept a file of questions. **Parsing is backend work** — the mock declares the contract and returns fixture data (`Watchlist_Qs.xlsx` → its 14 questions; any other file → an honestly labeled placeholder list). The real `parseQuestionFile` (and `addQuestionSet` when given only a file) must:

- read **.xlsx: the first sheet, the first column (A), one question per row** (CSV: the first column, one per row);
- keep **row order**; skip blank cells; trim whitespace; do not treat any row as a header — if the file carries one, it reads back as a question and the user removes it in the review card;
- return the questions to the client and **retain nothing** — the file is read for review, not stored. Nothing persists unless the user saves the questions as a set (`addQuestionSet` with `questions`);
- reject a wrong type or an unreadable/empty file with a message naming the file (the UI shows it verbatim, amber).

The upload itself goes through the shared upload states (per-file progress; over-limit and wrong-type rejections named, amber; the limit is `MAX_UPLOAD_MB` in `src/components/upload/config.ts`). A real sample lives at `docs/demo-files/Watchlist_Qs.xlsx`.

## Applications that share a workflow — Inquiry (v1.8)

**Inquiry is CPEA with one configuration flag off, not a second codebase.** The registry (`src/apps.ts`) gives every application a `config`; Inquiry's is `{ questionSets: false }`. The CPEA screens (start · processing · results · runs · documents, `src/screens/erm/`) are mounted twice — at `/erm` for CPEA and `/inquiry` for Inquiry — and read which application they are from a context the shared shell provides (config, base path, copy overlay, capability slots). CPEA's route configuration wires the question-set control into its slot; Inquiry's never reaches it (a unit test walks Inquiry's import graph to prove it).

For the http implementation this means:

- **The same seam, scoped by application.** `startRun` / `getRun` / `listRuns` / `cancelRun` take `app` (`'erm'` | `'inquiry'`). Each application keeps **its own run store** — Inquiry's runs never list in CPEA and vice versa; a runId does not resolve across stores.
- **Inquiry asks one question per run.** Every Inquiry run is prompt-only (answers keyed `questionId: 'prompt'`) and renders the single-question results shape. `questionSetId` on an Inquiry run rejects.
- **Everything else is shared**: `searchBorrowers`, `resolvePopulation` (including the borrower scope), `searchRepository`, the document viewers, the grading contract.
- **Question-set shelves are per-application** too (`getQuestionSets(store)`): CPEA's and Vantage's never mix; Inquiry has none.
...
- The app stores only UI preferences in `localStorage` (`sentinel.theme`, `sentinel.textSize`, `sentinel.rail.collapsed`, `sentinel.ctx.collapsed`, `sentinel.ctxWidth`, `sentinel.lastApp`). `sentinel.mock.state` and `sentinel.mock.user` exist only in mock mode. (The v1.6 removal of the extraction settings also removed `sentinel.extraction.settings`.)
