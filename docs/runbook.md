...
| In a Domino workspace, nothing appears after `make dev` / `make run` | Nothing auto-opens there. Both print (or `make doctor` prints) the URL to visit: your workspace URL with `/proxy/<port>/` appended. |
| In a Domino workspace, the page loads but is blank | The workspace is not pinned, so the build used laptop defaults and its root-absolute assets 404 behind the prefix. Run `echo '<paste workspace URL>' > .domino-workspace-path`, then `make build`. `make doctor` reports the active mode and which kind of build `dist/` holds. |
