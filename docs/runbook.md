...
| In a Domino workspace, the page loads but is blank | `dist/` was built outside the workspace, so its assets are root-absolute and 404 behind the prefix. Rebuild inside the workspace: `make build`. `make doctor` reports which kind of build `dist/` holds. |
