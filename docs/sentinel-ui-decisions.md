...
Steve's running keep/cut ledger for the Sentinel UI redesign, maintained across design-workshop sessions. Living mockup: https://claude.ai/code/artifact/2c5b4187-08cf-4fd6-89da-1d305d7c2ecf (file: `sentinel-mvp-concept.html`). Last updated 2026-09-09. **Status: HANDED OFF — `main` tagged `v1.2.1` (masthead S/M/L text-size control — `--user-scale` compounding with `--root-size`, persisted pre-paint like the theme; `v1.2` display portability over `v1.1.2`: bundled Inter Variable + Source Serif 4 Variable, rem type scale off one `--root-size` knob with a low-DPI step, low-DPI rule/ink token darkening; `v1.1.2` fictional-name hygiene over `v1.1.1`: vetted-fictional demo names, coined generated rows, fictional-data disclaimer, STATE_VERSION 5; `v1.1.1` patched `v1.1-handoff`: canonical feeder copy, design-artifact reconciliation, `--muted` restore; `v1.1-handoff` superseded `v1.0-handoff`: stale-state fixes, error boundary, schema versioning, structural assessment areas, canonical section titles), certified on a literal clean clone (make install && make build && make run), full chain green (97 unit, 86 Playwright browser mode + hash mode verified, 3 Flask). This tag is what the development team inherits.** Design rounds shipped along the way: v0.1-handoff → v0.3-domino-patterns → v0.4-copy-and-setup → v0.5-doc-browser → v1.0-handoff (assessment layer + reference data); the environment/portability round merged untagged between v0.1 and v0.3. This ledger plus the mockup and the build spec are the sources of truth. Resolved 2026-09-09: Stone light `--muted` darkened #78716c → #746d67 (4.75:1 on `--bg-subtle`); contrast audit regenerated and the mockup's `:root` synced.
...
- **Areas of assessment** (added 2026-09-09 from a real workpaper example): the management-summary verdict layer — each major review topic (transaction structure/underwriting, repayment capacity primary/secondary, portfolio management, regulatory classification, PD, LGD, traded products) with a rating disposition (satisfactory / unsatisfactory / n-a / pending) and a reason narrative. Sits between the attention block and the workpaper; the whole zone is collapsible (like the attention block) with the pending count remaining visible on the collapsed header, and each area row collapses individually. Ratings are analyst dispositions (AI-suggested once reasoning is live; advocate/dissent feed contested ones), recorded to the review record; a pending area names its blocker and deep-links to the section; the exported review opens with this table (screen = deliverable). Structural to every review (clarified 2026-09-09): all reviews carry the zone — completed reviews fully rated, read-only reviews without action buttons; a missing-areas state is a data-integrity guard, never an expected state.
...
- Inline collapsible workpaper: one document flow, serif section titles on hairline rules. (Supersedes the section card grid and the separate drill-in screen.) Canonical section titles (corrected 2026-09-09; the earlier six were indicative POC-era names): 1 Borrower / Counterparty & Relationship Overview · 2 Financials · 3 Underwriting & Documentation · 4 Portfolio Management · 5 Risk Rating Accuracy · 6 Trading Activity & Exposure Analysis. There is no 'Recommendation' section — adjudication lives in the areas of assessment.
...
- Fictional-data hygiene (2026-09-09): all demo names are vetted-fictional, collision-checked; canonical set Veyland/Ambervale/Seldwyn/Northgale/Farrowdale/Verloway, sponsor Bramhurst; generated rows use coined non-colliding names; README and fixtures carry the fictional-data disclaimer. Policy citations use the illustrative ProcMan-DEMO id — real internal document numbers never appear with illustrative text.
