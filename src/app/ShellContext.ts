 ...
 * - the selected work-paper item (exactly one), which drives the right rail.
 */
...
export function useRegisterCurrentReview(review: CurrentReview | null): void {
  const { setCurrentReview, setSelectedItemId } = useShell()
  useEffect(() => {
    setCurrentReview(review)
  }, [review, setCurrentReview])
  // Clear on unmount only — a re-fetched review must not reset the selection.
  useEffect(
    () => () => {
      setCurrentReview(null)
      setSelectedItemId(null)
    },
    [setCurrentReview, setSelectedItemId],
  )
}
